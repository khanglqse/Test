...
export default class CampaignService {
    static getListCampaign(param) {
        const result = ServiceRepository.getAPI(`${resource}?PageNumber=${param.pageIndex}&PageSize=${param.pageSize || 5}&Keyword=${param.keyword || ""}&SalaryFrom=${param.budget?.from}&SalaryTo=${param.budget?.to}`);
